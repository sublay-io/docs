...
<ParamField body="include" type="string">
  Comma-separated associations to populate: `"files"` (message attachments) and `"grants"` (each message's [reputation-grant summary](/data-models/reputation-grant#grantsummary)). Combine them as `"files,grants"`.
</ParamField>
...
<ParamField body="include" type="string">
  Comma-separated associations to populate. Only `"grants"` is supported here — it attaches the message's [reputation-grant summary](/data-models/reputation-grant#grantsummary) at `message.grants`.
</ParamField>
...
<ParamField body="gif" type="GifData | null">
  A [GifData](/data-models/chat-message#gifdata) attachment, or `null` to clear it.
</ParamField>
...
and cannot report their own message. See [createReport](/js-sdk/reports).
...
Sets or clears the signed-in user's mute on a conversation. Muting suppresses the `message` push for this conversation only; the message is still delivered and readable. See [Notification Preferences & Mute](/notification-preferences).
...
<ParamField body="duration" type='"8h" | "24h" | "1w" | "forever" | null' required>
  The mute **duration choice**, or `null` to clear the mute. Send the choice, never a raw timestamp — the server resolves the window.
</ParamField>

**Returns** — `Promise<{ currentMember: ConversationMember }>` — the signed-in user's own member row.
