...
`false` means the unbind was not skipped — it either succeeded or was never asked for,
as with a sign-out that passed no `pushDevice`. Both skips also log a warning, so a
caller that ignores the result still hears about it.
...
<ParamField body="pushDevice" type="object">
  This device, so its push binding survives the change. Same value you pass to `push.register` and `auth.signOut`: `{ platform: "ios" | "android", token }` or `{ platform: "web", subscription }`.
