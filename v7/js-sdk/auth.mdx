...
<ParamField body="pushDevice" type="PushDeviceIdentifier">
  ...
  **It requires a refresh token.** The unbind is scoped to the refresh token's
  subject — that is what stops one account removing another's binding on a shared
  device — so with no token to resolve, the server returns `204` without ever
  reading `pushDevice`. Rather than report a success that unbound nothing, this
  call **throws** before issuing the request.
  ...
  refresh token. Pass `refreshToken` alongside `pushDevice` there. A sign-out that
  asks for no unbind is unaffected and still works with no credential at all.
</ParamField>
...
A password change ends **every other session** for that user and deletes every push binding they hold — see [Change Password](/api-reference/auth/change-password). The session making the call survives only if it names itself.
...
<ParamField body="refreshToken" type="string">
  The session to keep. Defaults to the SDK's stored refresh token in SDK-managed mode, so you normally do not pass it.

  In host-managed mode the SDK holds no refresh token, so pass the one your host holds. With no refresh token resolvable, the password still changes and **every** session ends including this one — the fail-secure default, not an error. Your next request then `401`s and the host re-authenticates.
</ParamField>
