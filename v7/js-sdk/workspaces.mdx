...
The bearer-token user's resolved standing on a workspace — `{ reasons, capabilities, permissions, rank, relativeRank }`. **Self-service**, so nothing is fenced here. (`relativeRank` is degenerate on this read — an offset from you to yourself: `0` when you hold a member row here, `null` when you do not.) Drive your UI off this rather than guessing: a permission check is a one-line `.includes()` on the result.
