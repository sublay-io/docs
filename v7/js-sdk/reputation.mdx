...
<Note>
  **Grants on chat messages are private to their conversation.** With `targetType: "chat-message"` the list is populated only when the token's user is an active member of that conversation; everyone else gets an empty list and a `{ total: 0, count: 0, viewerTotal: 0 }` summary rather than an error. The `recipientId` / `senderId` shapes never carry chat-message-targeted grants at all, so a "reputation I received" feed shows entity, comment and untargeted grants only.
</Note>
