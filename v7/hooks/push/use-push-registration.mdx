...
- **marks accounts as needing a re-bind**, so each is bound the next time the user switches into it, using the live session that switch establishes. Two things put an account in that state, and they are different sets:
  ...
  A repeat `register()` on the same identifier that flips nothing marks nothing.
...
| `reactNativePushTokenAdapter` | Firebase `onTokenRefresh`, also rotation-only. The event carries an **FCM** token while the adapter registers **APNs** on iOS, so the handler ignores the emitted value and re-derives through `getDeviceIdentifier()`. Consequence: iOS APNs rotation is not reported by this event at all. It is caught by the [mount-time read](#reading-the-current-identifier-on-mount-native) on the next launch instead — this adapter declares `canReadIdentifierWithoutPrompting` — or sooner by a `register()` or `unregister()`. A failed re-derivation is reported through the SDK's error handler rather than passed off as "no change". |
