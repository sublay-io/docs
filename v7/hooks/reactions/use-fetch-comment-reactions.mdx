...
<Note>
  Authentication is optional. The endpoint works without a token, but when the
  caller is signed in, reactions from users they have blocked are excluded from
  both the results and the count.
</Note>
