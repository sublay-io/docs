...
<ParamField path="spaceReputationId" type="string">
  Resolves each reactor's reputation into a `spaceReputation` number on the embedded `user`. Accepts a space `<uuid>`, `"none"` (project-general bucket), or `"context"` (server-derived from the request context). Empty string, `"general"`, and `null` are rejected. See [Reputation](/data-models/reputation).
</ParamField>
...
<ResponseField name="data" type="Reaction[]">
  Array of reaction records. Each includes `id`, `userId`, `reactionType`, `targetId`, `targetType`, `createdAt`. The `user` field is populated when user data is available; when `spaceReputationId` is set it carries an added `spaceReputation` number. See [Reputation](/data-models/reputation).
</ResponseField>
