...
<Warning>
  **`capabilities`, `permissions`, `rank` and `relativeRank` are fenced.** They are **omitted from the response** (absent — not `null`) unless the signed-in user holds one of the four people-operating capabilities on this workspace (`invite`, `remove-member`, `edit-member-access`, `edit-member-profile`), is the `owner` / an `ancestor-owner`, or is reading **their own** standing. The four fields are optional in the SDK type — read them defensively (`standing.capabilities ?? []`). Everything else (`user`, `reasons`, `title`, `metadata`) is visible to any relation. `relativeRank` is fenced *with* `rank`, not beside it: it is `rank` minus your own anchor, a number you already know, so leaking it would leak `rank` exactly.
</Warning>
