...
<Warning>
  **Your project must have `workspaceInviteAcceptUrl` set**, exactly as [`useCreateWorkspaceInvite`](/hooks/workspaces/use-create-workspace-invite) requires — resending an email with nowhere to deep-link is equally pointless. Without it the call fails with `409 workspace/missing-invite-accept-url` and `expiresAt` is left untouched. See [Project settings](/sdk/workspaces/overview#project-settings).
</Warning>
...
| `workspace/missing-invite-accept-url` | 409 | The **project** has no [`workspaceInviteAcceptUrl`](/sdk/workspaces/overview#project-settings) configured. The invitation row is not touched. |
