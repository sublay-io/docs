...
<ParamField body="spaceReputationId" type="string">
  Resolves the reported content's author reputation into a `spaceReputation` number on the embedded `target` author. Accepts a space `<uuid>`, `"none"` (project-general bucket), or `"context"` (server-derived from the request context). Empty string, `"general"`, and `null` are rejected. See [Reputation](/data-models/reputation).
</ParamField>
...
<ResponseField name="target" type="Entity | Comment | null">The reported content record (includes the author user object). Populated even if the content was soft-deleted. When `spaceReputationId` is set, the embedded author carries an added `spaceReputation` number. See [Reputation](/data-models/reputation).
