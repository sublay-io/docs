...
<ResponseField name="target" type="Entity | Comment | null">The reported content record (includes the author user object). Populated even if the content was soft-deleted. When `spaceReputation` is requested, the embedded author carries an added `spaceReputation` number. See [Reputation](/data-models/reputation).
