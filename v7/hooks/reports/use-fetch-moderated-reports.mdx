...
<ParamField body="targetType" type='"comment" | "entity" | "message"'>
...
<ResponseField name="targetId" type="string">ID of the reported entity, comment, or chat message.</ResponseField>
<ResponseField name="targetType" type='"comment" | "entity" | "message"'>Type of the reported content.</ResponseField>
...
<ResponseField name="target" type="Entity | Comment | ChatMessage | null">The reported content record (includes the author user object). Populated even if the content was soft-deleted. When `spaceReputation` is requested, the embedded author carries an added `spaceReputation` number. See [Reputation](/data-models/reputation).
...
- Message reports appear in this queue only for conversations attached to a space. DM and group chat reports have no space, so they are project-level and appear only in the Sublay dashboard — see [`useCreateReport`](/hooks/reports/use-create-report).
