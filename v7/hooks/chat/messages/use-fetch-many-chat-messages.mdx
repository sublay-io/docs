...
<ParamField path="spaceReputationId" type="string">
  Resolves each message sender's reputation into a `spaceReputation` number on the embedded `user`. Accepts a space `<uuid>`, `"none"` (project-general bucket), or `"context"` (server-derived from the request context). Empty string, `"general"`, and `null` are rejected. See [Reputation](/data-models/reputation).
</ParamField>
...
<ResponseField name="messages" type="ChatMessage[]">
  Messages for the requested page. When `spaceReputationId` is set, each embedded sender carries an added `spaceReputation` number. See [ChatMessage](/data-models/chat-message) and [Reputation](/data-models/reputation).
</ResponseField>
