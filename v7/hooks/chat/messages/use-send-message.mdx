...
<ResponseField name="ChatMessage" type="ChatMessage">
  The server-confirmed [ChatMessage](/data-models/chat-message) object. When `spaceReputation` is requested, its embedded sender carries an added `spaceReputation` number. See [Reputation](/data-models/reputation).
</ResponseField>
