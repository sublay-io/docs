...
<ResponseField name="members" type="ConversationMember[]">
  The current member list. When `spaceReputationId` is set, each embedded user carries an added `spaceReputation` number. See [ConversationMember](/data-models/conversation-member) and [Reputation](/data-models/reputation).
</ResponseField>
