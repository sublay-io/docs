...
<ResponseField name="changePassword" type="function">
  ...
  **Every other session for that user ends, and the caller's does not.** The hook
  sends the active account's stored refresh token with the request, which is how
  the server knows which session to spare — the route is authenticated by an
  access token, which names no session. The user stays signed in where they are
  standing; every other device must sign in again with the new password. The
  change also clears that user's push bindings on every device; the calling
  device re-binds from its live session and the others re-bind when next opened.
  See [Change Password](/api-reference/auth/change-password).

  Other accounts stored on **this** device are killed by it too, and a stored
  account killed that way still has a valid-looking expiry, so it is only
  discovered by trying — which is what `needsReauth` on
