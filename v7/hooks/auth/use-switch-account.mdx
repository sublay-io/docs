            ...
            <button
              onClick={() =>
                switchAccount({ userId: account.id }).catch(() => {
                  /* Rejects when the stored credential is refused; `error` below
                     carries the reason and you stay signed in where you were. */
                })
              }
...
function SwitchButton({ userId }: { userId: string }) {
  ...
  return (
    <Button
      title="Switch"
      onPress={() =>
        switchAccount({ userId }).catch(() => {
          /* Read `error` from the hook; without a catch this is an
             unhandled rejection. */
        })
      }
      disabled={isSwitching}
