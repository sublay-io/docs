...
<Note>
  **Chat-message targets are private to their conversation.** With `targetType: "chat-message"`, the list is populated only when the logged-in user is an active member of that message's conversation; for anyone else — including someone who has left it — `grants` stays empty and `summary` comes back as `{ total: 0, count: 0, viewerTotal: 0 }` rather than erroring. And the `recipientId` / `senderId` shapes never include chat-message-targeted grants at all, for any caller. See [useFetchManyReputationGrants](/hooks/reputation/use-fetch-many-reputation-grants#chat-message-targets).
</Note>
