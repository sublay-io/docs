...
Authentication is optional — the endpoint works without a token. When the caller is signed in, reactions from users with a block between them and the caller, in either direction, are excluded from both the results and the count. Block filtering requires the `moderation` bundle.
