...
<ParamField query="include" type="string">
  Comma-separated list of associations to include. Valid values: `user`, `entity`, `space`, `grants` (the [reputation-grant summary](/data-models/reputation-grant#grantsummary). Note `parent` is accepted by validation on this endpoint but is only resolved on the single-comment reads.)
