...
<AccordionGroup>
  ...
  <Accordion title="Quota Reached — 429">
    ```json
    {
      "error": "Quota reached for apiCallsPerMonth. Current: …, Action size: …, Limit: …",
      "code": "project/quota-reached",
      "details": { "current": 0, "actionSize": 0, "limit": 0, "metric": "apiCallsPerMonth" }
    }
    ```
    Returned when the project has spent its monthly API-call allowance and is on
    a plan that hard-blocks at the limit. Distinct from the rate-limit `429`
    below, which is plain text with no `code`.
  </Accordion>
