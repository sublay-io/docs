...
<AccordionGroup>
  <Accordion title="Push Deregistration Failed — 500">
    ...
    ```
    The push binding could not be deleted, so nothing was committed and the
    session was left intact. Either the delete itself failed inside the
    transaction, or the server could not even determine whether the project has
    the `push` bundle — both leave a binding that may still be live, so keep the
    account and its credential and retry.
  </Accordion>
...
<Note>
  **A rejection that never reached this endpoint is not an unbind failure.** The `423`, `503` and
  ...
  unremovable whenever the project is throttled or migrating.
</Note>
