...
<ParamField query="blockedFilter" type="string" default="exclude">
  Controls how entities authored by users the viewer is [block-edged](/sdk/moderation/overview#blocking) with are treated. `"exclude"` (default) hides those entities. `"include-outbound-blocked"` re-includes entities authored **only** by users the viewer themselves blocked (their own "view anyway" choice); it never surfaces content from users who blocked the viewer. Counts reflect what the viewer sees (computed post-filter). Has no effect when the `moderation` bundle is not installed.
</ParamField>
...
- [Reputation data model](/data-models/reputation) — the `spaceReputation` object contract
