...
<ParamField query="spaceReputationId" type="string">
  Adds `spaceReputation` for the referenced bucket. A space ID returns that space's bucket; empty, `null`, or `general` returns the project-general bucket. Missing buckets read as `0`.
</ParamField>

<ParamField query="spaceReputationDescendants" type="boolean">
  Only valid with a real space ID. When `true`, `spaceReputation` is the subtree sum — the space plus all of its descendants (the root space's own bucket included).
</ParamField>
