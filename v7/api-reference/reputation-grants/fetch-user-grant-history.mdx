...
<ParamField header="x-sublay-project-id" type="string" required>
  The project UUID. Required for **every** caller, including service and master keys — it is how the server resolves which project's key you are presenting. Without it a raw project key is parsed as a dashboard session token and rejected with `403`.
</ParamField>
...
A service key is presented as the bearer credential alongside `x-sublay-project-id`. Dashboard sessions authenticate with their client JWT as usual.
...
<AccordionGroup>
  <Accordion title="Invalid Params — 400">
    ```json
    { "error": "userId: Invalid UUID", "code": "user/invalid-params" }
    ```
  </Accordion>
