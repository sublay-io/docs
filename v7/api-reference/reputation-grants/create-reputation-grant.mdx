...
<AccordionGroup>
  ...
  <Accordion title="Tables Not Available — 403">
    ...
    ```
    The `reputation` bundle is missing, the target's bundle is (e.g. a `chat-message` target without `chat`), or a `spaceId` was supplied on a project without `spaces`. `missingTables` names exactly what is absent.
  </Accordion>
