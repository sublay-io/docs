...
<AccordionGroup>
  ...
  <Accordion title="Insufficient Permissions — 403">
    ...
    ```json
    { "error": "Moderators can only ban regular members.", "code": "space/insufficient-permissions" }
    ```
  </Accordion>
  ...
  <Accordion title="Last Admin — 400">
    ```json
    { "error": "Cannot ban the last admin. Promote another member to admin first.", "code": "space/last-admin" }
    ```
  </Accordion>
</AccordionGroup>

See also: [useBanMember](/hooks/spaces/use-ban-member), [Unban Member](/api-reference/spaces/membership/unban-member)
