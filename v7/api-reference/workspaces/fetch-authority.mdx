...
- There is **no `relativeRank`** on this read. It is an offset from the caller, and here the caller *is* the subject — so it could only ever be `0`, a constant dressed as a coordinate. `rank` is the position this endpoint reports. `relativeRank` is meaningful on the [roster](/api-reference/workspaces/members/list-members) and [member-standing](/api-reference/workspaces/members/fetch-member-standing) reads, where the subject is somebody else.
