...
<Warning>
  **Requires `workspaceInviteAcceptUrl` on the project.** The invitation email's only call to action is a deep-link built from that setting, and it has no default — so an unconfigured project cannot create invitations at all. Without it this endpoint returns `409 workspace/missing-invite-accept-url` and persists nothing (no invitation row, no email, no webhook).
...
<AccordionGroup>
  ...
  <Accordion title="Missing Invite Accept URL — 409">
    ```json
    { "error": "This project has no workspaceInviteAcceptUrl configured...", "code": "workspace/missing-invite-accept-url" }
    ```
    A **project misconfiguration**, not a bad request — the payload is fine and the caller is authorized. Set [`workspaceInviteAcceptUrl`](/sdk/workspaces/overview#project-settings) in your project settings and retry the identical request.
  </Accordion>
</AccordionGroup>
