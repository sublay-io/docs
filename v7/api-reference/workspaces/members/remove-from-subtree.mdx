...
<ParamField body="actingUserId" type="string" required>
  Service/master keys only — the **acting** user (must hold `remove-member`, rank-bounded per node). **Required** for a key: there is no unbounded path on this route, so the sweep's reach is always that user's — a key that names nobody is refused with `400 workspace/missing-user-id`. Name the **owner** for a full-subtree sweep.
