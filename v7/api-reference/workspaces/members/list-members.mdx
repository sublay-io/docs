...
<Warning>
  **There is no `page` or `limit` on this route, and sending them does nothing.** They are not rejected either — no error, no effect. A client that paginates receives the **entire** roster every time and mistakes it for page 1, so a naive "fetch until a short page" loop never terminates and a "page 2" request re-renders page 1's contents.
