...
| `rank` | `number` | The initial [rank](/data-models/workspace-member#rank-semantics) to apply on accept — always **absolute**, whichever coordinate the inviter used. A [`relativeRank`](/data-models/workspace-member#two-coordinates-rank-and-relativerank) offset (or the default, one rung below the inviter) is resolved to a number when the invitation is created and **frozen** here; it does not follow the inviter's rank afterwards. |
