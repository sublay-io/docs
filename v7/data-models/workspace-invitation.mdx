...
<Note>
  **Delivery requires one project setting.** Invitation emails deep-link to your app via [`workspaceInviteAcceptUrl`](/sdk/workspaces/overview#project-settings), which has **no default**. Until it is set, creating or resending an invitation is refused with `409 workspace/missing-invite-accept-url` — including for invitees who already have an account. The link itself carries nothing secret; acceptance is identity-matched.
</Note>
