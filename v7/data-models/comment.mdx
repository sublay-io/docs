...
<Note>
  The two single-comment reads ([Fetch Comment](/api-reference/comments/fetch-comment) and [Fetch Comment by Foreign ID](/api-reference/comments/fetch-comment-by-foreign-id)) nest the record under a `comment` key, so on those the summary is at `comment.grants`. The comment feed returns comments bare.
</Note>
