...
| `inheritsFromParent` | `boolean` | Whether capabilities held on an ancestor **reach** into this node. **Off by default (strict).** Server-set at creation from the project default; owner-only to flip afterwards. See [Reach & the wall/door rule](#reach--the-wall-door-rule). |
