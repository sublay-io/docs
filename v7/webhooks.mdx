...
Sublay can call your server when things happen in your project. There are two webhook systems:
...
When the digest is enabled for a space, Sublay sends a daily summary of the past 24 hours of activity to that space's digest webhook URL. The cron runs hourly and delivers the digest at the configured schedule hour in the space's timezone.
