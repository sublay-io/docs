...
<Warning>
  **Absent, not falsy — and nothing actor-shaped on the unbounded path.**
  ...
  actor — a name containing `user` or `acting`, or a UUID value under any name.
  ...
  for every caller (the invitee on `createWorkspaceInvite` is the sole
  exception), and a non-UUID id in a path is a `400 workspace/invalid-params`
