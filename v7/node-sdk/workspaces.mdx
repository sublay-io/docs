...
<Note>
  **Requires the `workspaces` bundle.** See [Bundles](/bundles) to install it. Because this is a **service-key** SDK, every call **accepts** an `actingUserId` — the user the action is performed **as** (the service key can [act as any user](/node-sdk/overview)). Whether it is required or optional is **per route** — see the note below. For `fetchWorkspaceAuthority`, `actingUserId` is the user whose standing is resolved.
</Note>
...
<Note>
  **On many routes the actor is required, not optional.** `actingUserId` must be
  supplied by a key on the routes whose result belongs to a specific person:
  ...
  `leaveWorkspace`, `updateWorkspaceMember`, `removeWorkspaceMember` and
  `removeWorkspaceMemberFromSubtree`. Omitting it there is a
  `400 workspace/missing-user-id`, never a bypass. Everywhere else it is
  optional and omitting it runs unbounded.
</Note>
...
<ParamField body="actingUserId" type="string" required>The user to act as — must hold `edit-workspace` on this workspace, or own it.</ParamField>
...
<ParamField body="workspaceId" type="string" required />
<ParamField body="actingUserId" type="string" required>The user to act as — must own this workspace or an ancestor.</ParamField>
...
<ParamField body="workspaceId" type="string" required />
<ParamField body="actingUserId" type="string" required>The user to act as — must own this workspace or an ancestor. This call cascade-deletes the whole subtree, so name the owner deliberately.</ParamField>
...
<ParamField body="previousOwnerRank" type="number">On demote, the ex-owner's rank (absolute only). Defaults to **one rung below the acting user's anchor** — `0` when the actor is apex (a key naming nobody, or an owner/ancestor owner with no member row here), or one below their own row when the named `actingUserId` sits in this workspace's ladder.</ParamField>
