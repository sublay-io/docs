...
<Warning>
  **Absent, not falsy — and no stray fields on the unbounded path.**
  ...
  acquiring full authority. And when a key names **nobody**, any unrecognized
  top-level field in the body or query is a `400` as well — a misspelled actor
  must not read as "no actor". A key that *did* name someone stays permissive,
  and `fetchWorkspace` is exempt (its schema strips unknown query params before
  the actor is read, so a typo there is silently dropped and the read runs
  unbounded — check that one yourself). Full list with the reasoning:
...
<ParamField body="actingUserId" type="string">Optional — the user to act as. Sent as a query param. Visibility resolves against them, so a key acting as someone with no relation to this workspace gets that user's `404`. Omit to read as the app itself. **Spell it carefully:** this endpoint's query schema strips unrecognized params before the actor is read, so a mistyped `actingUserId` is dropped silently and the read runs unbounded — no `400` warns you. See [Fetch Workspace](/api-reference/workspaces/fetch-workspace).</ParamField>
