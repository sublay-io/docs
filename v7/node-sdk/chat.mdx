...
Sets or clears the acting user's mute on a conversation. Muting suppresses the `message` push for this conversation only; the message is still delivered. See [Notification Preferences & Mute](/notification-preferences).
...
<Warning>
  Unlike the rest of `chat`, this endpoint is **acting-user-scoped** — the server derives the user from the token and rejects service/master keys. There is **no** `userId` / `actingUserId` param: a user only mutes their own membership.
</Warning>
...
**Returns** — `Promise<{ currentMember: ConversationMember }>` — the acting user's own member row.
