...
<Warning>
  These two endpoints are **acting-user-scoped** — the server derives the user from the token and rejects service/master keys. There is no `userId` parameter and no impersonation path; a user's own notification settings can only be read/written from that user's own session.
</Warning>

### getNotificationPreferences

Reads the acting user's disabled push types. Returns an empty array (all-on) when no preferences are set.
...
Replaces the acting user's disabled-types set (upsert). Send the **complete** set you want stored, not a delta; only valid event type names are accepted.
...
Setting or clearing a conversation mute lives on the **`chat`** module (it's a `/chat/...` route): [`chat.muteConversation`](/node-sdk/chat#muteconversation). Like the preference endpoints it is acting-user-scoped — no `userId` param — and takes a duration **choice**, never a timestamp.
