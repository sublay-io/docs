...
Requires the `reputation` bundle. A `chat-message` target additionally needs the `chat` bundle. Only `mintGrant` validates `spaceId` against the `spaces` bundle — on a transfer, an unusable `spaceId` simply finds no funded bucket and returns `409 reputation-grant/insufficient-reputation`.
...
<ParamField body="amount" type="number" required>
  Whole number of at least `1`. Never zero, negative, or fractional — use `mintGrant` to create or destroy reputation.
</ParamField>
<ParamField body="spaceId" type="string | null">
  The bucket both legs move in. Omitted or `null` = the project-general bucket. Reputation never crosses between the general bucket and a space bucket.
</ParamField>
...
Throws `409 reputation-grant/insufficient-reputation` when the sender's bucket is short — nothing moves. A grant is **not idempotent**: a retried call moves the points a second time.
