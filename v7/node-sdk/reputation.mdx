...
<Note>
  **Grants on chat messages are private to their conversation, and a service key does not lift that.** The `recipientId` / `senderId` shapes never carry chat-message-targeted grants, for any caller. The target shape does return them to a service key — it is the app's own backend, so it passes the membership gate that an ordinary user token has to satisfy — but with no viewer identity, so `summary.viewerTotal` is always `0`.
</Note>
