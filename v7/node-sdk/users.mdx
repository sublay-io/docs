...
User-to-user **blocking** is part of the trust & safety [moderation bundle](/sdk/moderation/overview#blocking). These functions require that bundle; a project without it returns `403 database/tables-not-available`. Each acts on behalf of one user (the blocker) — pass that user's ID as `actingUserId`.
...
```typescript
const block = await sublay.users.createBlock({
  userId: "usr_target",
  actingUserId: "usr_blocker",
});
```
...
**Returns** — `Promise<Block>` — `{ id, blockerId, blockedId, createdAt }`.
...
```typescript
await sublay.users.deleteBlock({
  userId: "usr_target",
  actingUserId: "usr_blocker",
});
```
...
**Returns** — `Promise<{ blocked: boolean; blockId?: string; createdAt?: string }>`
...
**Returns** — `Promise<PaginatedResponse<BlockedUser>>`, where each `BlockedUser` is `{ id, blockedUser, createdAt }`.
