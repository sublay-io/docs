...
A few routes on the `users` module act on one user toward another — following someone, requesting a connection. These are nested under the target user's ID in the path, so the **target** is `userId` and the **actor** is passed separately as `actingUserId`.

<Note>
  `chat`, `storage`, and `oauth` are intentionally not exposed by this SDK.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Users" icon="user" href="/v7/node-sdk/users">
    Fetch and update profiles, follow and connection actions
  </Card>
  ...
  <Card title="Hosted Apps" icon="cube" href="/v7/node-sdk/hosted-apps">
    Fetch hosted app configuration
  </Card>
</CardGroup>
