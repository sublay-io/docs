...
Resolves a reported chat message in a space conversation by applying one or more actions.
...
<ParamField body="userId" type="string">
  The user to ban — the ban **target**, not the moderator. Required when `actions` includes `"ban-user"`.
</ParamField>
...
<ParamField body="actingUserId" type="string">
  The moderator the action is attributed to. Attribution only — this route is service-key god-mode, so omit it for a backend/system action.
</ParamField>

<Note>
  `ban-user` is space-scoped: it sets the target's space membership to banned.
  Project-wide suspension is a separate dashboard action.
</Note>
