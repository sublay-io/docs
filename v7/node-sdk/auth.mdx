...
A password change ends **every session** for that user and deletes every push binding they hold, unless one session is named to be spared — see [Change Password](/api-reference/auth/change-password).
...
<ParamField body="refreshToken" type="string">
  The session to spare, named by the refresh token that device holds — typically one your own client just sent you. Its family survives while every other device must sign in again.

  Omit it and every session for the user ends, which is usually right for a server-side caller acting on someone's behalf: it holds no refresh token, and revoking everything is the fail-secure outcome rather than an error. A token that fails verification, or belongs to a different user, is ignored — it can never preserve someone else's session.
</ParamField>
