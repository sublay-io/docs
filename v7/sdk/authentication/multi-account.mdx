...
When the OS rotates the device's push token, every binding on the old token is dead. Wherever `usePushRegistration` is mounted, the SDK notices and re-binds each **enabled** stored account onto the new token with no further app involvement, leaving silenced accounts silenced. Detection lives in that hook and is gated on this device having registered at least once, so an app that never mounts it falls back to the next `register()`. See [Automatic token rotation](/hooks/push/use-push-registration#automatic-token-rotation) for the per-platform detail.
