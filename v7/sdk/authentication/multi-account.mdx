...
Each account in `accounts` is a `StoredAccount`: the profile summary — `id`, `name`, `username`, `email`, `avatar` — plus two re-auth markers, `tokenExpiresAt` and `needsReauth`.
...
| `needsReauth` | Reactive — set when a switch into that account is actually refused | Everything expiry cannot see: reuse detection, a password change, a remote sign-out-all, an admin revocation. All of those kill the token family while `exp` is still far in the future |
...
try {
  ...
  // The switch did not happen, and the account you were using is still
  // signed in. `error` is also set on the hook. Prompt a re-auth for `userId`.
}
```

<Warning>
  **Behavior change from the first hardening release.** A failed switch used to
  sign you out of the account you were already using: the teardown ran first, so
  by the time the dead credential was discovered there was nothing left to
  restore, and only the *selection* could be rolled back. Validating first
  removes that entirely. If your app has a "recover from a failed switch"
  branch that assumes the session is gone, it no longer fires.
</Warning>
...
`accountNeedsReauth(entry)` is exported alongside `isAccountPushEnabled` for reading the marker off a raw `AccountEntry`, since absent and `false` mean the same thing there.
