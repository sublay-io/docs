...
| Value | Description |
|-------|-------------|
| `"include-all"` | **Default.** No NSFW predicate — all entities returned regardless of NSFW status. |
| `"exclude"` | Drops effective-NSFW entities. |
| `"only"` | Returns only effective-NSFW entities. |

```tsx
// Hide NSFW content from the feed
fetchEntities({ nsfwFilter: "exclude" }, { sortBy: "hot" });
```

Sublay only labels (`nsfw`) and filters (`nsfwFilter`) NSFW content — it does not blur, age-gate, or store a per-viewer preference. Presentation is your app's responsibility. See [Entity NSFW flagging](/data-models/entity#nsfw-flagging).
