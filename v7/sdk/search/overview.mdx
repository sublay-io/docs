...
<Note>
  **Requires the `ai-search` bundle.** Semantic search and the AI ask endpoint are only available when the `ai-search` bundle is installed on your project. See [Bundles](/bundles) to add it.
</Note>
