...
<ParamField path="includeChildSpaces" type="boolean" default="false">
  Only applies with `spaceId`. When `true`, results also cover every space nested under it — the whole subtree at any depth — not just the named space. Ignored without `spaceId`.
</ParamField>
