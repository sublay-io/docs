...
If the sender's chosen bucket is short, the request rejects with `409 reputation-grant/insufficient-reputation` and nothing moves. Grant creation is **not idempotent** — guard your button against double-submits, because a retry moves the points twice.
