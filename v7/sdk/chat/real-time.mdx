...
| `message:grant` | A [reputation grant](/data-models/reputation-grant) landed on the message. Updates the message's `grants` summary in the store. The payload carries `{ messageId, conversationId, grant, summary }`, and `summary` is `{ total, count }` **only** — `viewerTotal` is per-viewer, so the SDK derives it from `grant.senderId` and `grant.amount`. Negative grants are never broadcast |
