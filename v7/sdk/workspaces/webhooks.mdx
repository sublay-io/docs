...
| `workspace.member.added` | Any membership **row** is created. |
| `workspace.member.removed` | Any membership **row** is deleted. |
...
- **`member.*` also covers non-invite roster changes** that have **no** invite event:
  - `member.removed` fires on a **leave**, a **remove**, a **remove-from-subtree** sweep, **user-deletion cleanup**, and when a user *becomes owner* and their existing member row is dropped (owner/member rows are disjoint).
  - `member.added` fires on the **transfer-demote** case (a previous owner demoted into a fresh member row).

So if you dropped `member.*` in favor of `invite.*`, you would lose every removal transition (leaves, removals, offboarding, deletions) — which is why `member.*` is the stream to mirror your live seat/roster set.
