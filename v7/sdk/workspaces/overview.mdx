...
<Note>
  **What actually needs a service key: acting on behalf of a *different* user.** Several endpoints accept an actor field, `actingUserId`, so a service/master key can perform an action *as* some named user. That parameter is **[node SDK](/node-sdk/workspaces) only** — `@sublay/core` and `@sublay/js` never expose it, because a client's actor is always the token's own user. (There is no client-side strip: a call that hand-writes `actingUserId` past the types is bound by the same rule as any user token — naming a user other than the token's own is rejected with a `403`.) This is the *only* difference in reach between the client SDKs and the node SDK for this bundle; it is **not** a restriction on which operations a client may perform.

  **Four endpoints do not enforce it yet.** On [`delete-workspace`](/api-reference/workspaces/delete-workspace), [`update-workspace`](/api-reference/workspaces/update-workspace), [`update-inherit-flag`](/api-reference/workspaces/update-inherit-flag) and [`transfer-ownership`](/api-reference/workspaces/transfer-ownership), the guard grants a key full authority *before* it resolves the named user, so the value is discarded and the call runs with owner authority no matter who you name. Naming a non-owner will not stop a destructive call. Each of those pages carries the details; enforcement lands in a follow-up change.
