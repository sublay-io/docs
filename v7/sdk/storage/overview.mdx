...
<Note>
  **Requires the `files-images` bundle.** File and image uploads are only available when the `files-images` bundle is installed on your project. See [Bundles](/bundles) to add it.
</Note>
...
- [Upload Image](/api-reference/storage/upload-image) — `POST /v7/:projectId/storage/images`
- [Upload File](/api-reference/storage/upload-file) — `POST /v7/:projectId/storage`
- [Fetch File](/api-reference/storage/fetch-file) — `GET /v7/:projectId/storage/:fileId`
- [Delete File](/api-reference/storage/delete-file) — `DELETE /v7/:projectId/storage/:fileId`
