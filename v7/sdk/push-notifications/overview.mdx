...
| Web | No in-page event exists (`pushsubscriptionchange` fires inside *your* service worker), so the SDK compares the browser's existing subscription against the stored one on mount. It never re-subscribes and never prompts — which also means web is the one platform where a device that has never stored an identifier can acquire one without calling `register()` again |
