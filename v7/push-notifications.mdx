...
- **Explicit logout:** call `unregister()` in your logout flow so the device stops receiving notifications for that account after sign-out. Other accounts bound to the same device are unaffected.
