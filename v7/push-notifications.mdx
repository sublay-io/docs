...
Once per device is enough. The SDK persists the device identifier and, wherever `usePushRegistration` is mounted, re-binds automatically when the OS rotates the token, so there is no "call it again on every launch to be safe" step — see [Device Lifecycle](#device-lifecycle).
...
- **Token rotation:** device tokens rotate (reinstall, OS refresh, backup restore), which kills every binding on the old token. The SDK detects this and re-binds every **enabled** account onto the new token with no app involvement beyond keeping `usePushRegistration` mounted — detection lives in that hook and is gated on this device having registered at least once. Rotation is picked up immediately on Expo and Android, and on the next page load on Web (which has no in-page rotation event). iOS APNs rotation under bare React Native is the one gap, and is picked up by the next `register()`. See [Automatic token rotation](/sdk/push-notifications/overview#automatic-token-rotation).
- **Signing out unbinds automatically:** once this device has registered, `signOut()`, `removeAccount()` and `signOutAll()` pass its identifier with the sign-out request, and the server deletes that account's push binding in the **same transaction** as the session teardown. If the unbind fails nothing commits — the sign-out fails and the account keeps its credential so the user can retry, rather than being left receiving notifications from an account it can no longer reach. A device that never registered sends no identifier, has no unbind to protect, and signs out exactly as it always did. Other accounts on the device are unaffected.
