...
Once per device is enough. The SDK persists the device identifier and re-binds automatically when the OS rotates the token, so there is no "call it again on every launch to be safe" step — see [Device Lifecycle](#device-lifecycle).
