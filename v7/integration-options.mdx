...
All endpoints are under `/:projectId/api/v7/`. Every request that requires a user identity must include a valid access token in the `Authorization: Bearer <token>` header.
